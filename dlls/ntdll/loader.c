...
{
    IMAGE_DOS_HEADER mz;
    // IO_STATUS_BLOCK io; // As Wine CX
    // LARGE_INTEGER offset; // As Wine CX
    ...
    if (!read_file( file, &mz, sizeof(mz), 0 )) return 0; // From Wine CX
    ...
    if (!read_file( file, &nt, sizeof(nt), mz.e_lfanew )) return 0; // From Wine CX
    ...
    *magic = nt.nt32.OptionalHeader.Magic;
    ...
    if (!read_file( file, &sec, count * sizeof(*sec),// From Wine CX
                    mz.e_lfanew + offsetof( IMAGE_NT_HEADERS32, OptionalHeader ) + nt.nt32.FileHeader.SizeOfOptionalHeader ))// From Wine CX
        return 0;// From Wine CX
    for (i = 0; i < count; i++)
    {
        ...
        // return io.Information;
        return read_file( file, buffer, min( maxlen, size ), sec[i].PointerToRawData + va - sec[i].VirtualAddress );// From Wine CX
    }
    return 0;
}
                               ...
                               SECTION_IMAGE_INFORMATION *image_info, struct file_id *id )
{
    ...
    IO_STATUS_BLOCK32 io32; // From Wine CX
    IO_STATUS_BLOCK io;
    LARGE_INTEGER size;
