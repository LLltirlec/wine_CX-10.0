...
static void set_dll_path(void)
{
    ...
    if (path) for (p = path, count++; *p; p++) if (*p == ':') count++; // From WineCX_24.0.5
    if (path_prepend) for (p = path_prepend, count++; *p; p++) if (*p == ':') count++; // From WineCX_24.0.5
...
static void preloader_exec( char **argv, const char *image_path ) // From WineCX_24.0.5
{
...
static NTSTATUS loader_exec( char **argv, WORD machine, const char *image_path ) // From WineCX_24.0.5
{
    // if (((argv[1] = get_alternate_wineloader( machine )))) preloader_exec( argv );
    if (((argv[1] = get_alternate_wineloader( machine )))) preloader_exec( argv, image_path ); // From WineCX_24.0.5

    argv[1] = strdup( wineloader );
    // preloader_exec( argv );
    preloader_exec( argv, image_path ); // From WineCX_24.0.5
...
NTSTATUS exec_wineloader( char **argv, int socketfd, const struct pe_image_info *pe_info, const char *image_path  ) // From WineCX_24.0.5
{
    ...
    return loader_exec( argv, machine, image_path ); // From WineCX_24.0.5
}
...
static const WCHAR *get_machine_wow64_dir( WORD machine )
{
    ...
    if (machine == native_machine || wow64_using_32bit_prefix) machine = IMAGE_FILE_MACHINE_TARGET_HOST; // From WineCX_24.0.5
...
DECLSPEC_EXPORT void __wine_main( int argc, char *argv[] )
{
    ...
    if (!getenv( "WINELOADERNOEXEC" ))  /* first time around */
    {
        ...
        if (pre_exec())
        {
            ...
            loader_exec( new_argv, current_machine, argv[0] ); // From WineCX_24.0.5
