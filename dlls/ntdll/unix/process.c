...
{
    int is_child_64bit = (pe_info->machine == IMAGE_FILE_MACHINE_AMD64 || pe_info->machine == IMAGE_FILE_MACHINE_ARM64); // From WineCX_24.0.5
    NTSTATUS status = STATUS_SUCCESS;
    int stdin_fd = -1, stdout_fd = -1;
    pid_t pid;
    char **argv;
    char *wineloader = NULL; // From WineCX_24.0.5
    const char *loader = NULL; // From WineCX_24.0.5
    char *image_path_name_a; // From WineCX_24.0.5
    int image_path_name_len; // From WineCX_24.0.5

    if (!is_win64 ^ !is_child_64bit) loader = get_alternate_loader( &wineloader ); // From WineCX_24.0.5
