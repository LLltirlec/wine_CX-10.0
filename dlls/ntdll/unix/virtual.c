...
static const char *get_prot_str( BYTE prot )
{
    ...
    buffer[3] = (prot & VPROT_WRITECOPY) ? (prot & VPROT_COPIED ? 'w' : 'W') // From WineCX
        : ((prot & VPROT_WRITE) ? 'w' : '-');
...
static DWORD get_win32_prot( BYTE vprot, unsigned int map_prot )
{
    ...
    ret = VIRTUAL_Win32Flags[vprot & 0x0f]; // From WineCX
                                    ...
                                    off_t offset, unsigned int vprot, BOOL removable )
{
    ...
    if (!removable || (flags & MAP_SHARED))
    {
        ...
        if (wine_mmap( (char *)view->base + start, size, prot, flags, fd, offset ) != MAP_FAILED) // From WineCX
            goto done;
...
static NTSTATUS map_pe_header( void *ptr, size_t size, int fd, BOOL *removable )
{
    ...
    if (!*removable)
    {
        ...
        if (wine_mmap( ptr, size, PROT_READ|PROT_WRITE|PROT_EXEC, MAP_FIXED|MAP_PRIVATE, fd, 0 ) != MAP_FAILED) // WineCX
            return STATUS_SUCCESS;
                                     ...
                                     int shared_fd, BOOL removable )
{
    ...
    if (machine && machine != nt->FileHeader.Machine && !wow64_using_32bit_prefix) return STATUS_NOT_SUPPORTED; // From WineCX
...
void virtual_map_user_shared_data(void)
{
    ...
    if ((res = server_get_unix_fd( section, 0, &fd, &needs_close, NULL, NULL )) ||
        ...
        (user_shared_data != wine_mmap( user_shared_data, page_size, PROT_READ, MAP_SHARED|MAP_FIXED, fd, 0 ))) // From WineCX
    {
...
{
    BOOL large_address_space_active = ((main_image_info.ImageCharacteristics & IMAGE_FILE_LARGE_ADDRESS_AWARE) || force_laa()); // From WineCX
    if (is_win64)
    {
        if (is_wow64())
            // user_space_wow_limit = ((main_image_info.ImageCharacteristics & IMAGE_FILE_LARGE_ADDRESS_AWARE) ? limit_4g : limit_2g) - 1;
            user_space_wow_limit = (large_address_space_active ? limit_4g : limit_2g) - 1; // From WineCX
    ...
    else
    {
        ...
        if (!large_address_space_active) return; // From WineCX
